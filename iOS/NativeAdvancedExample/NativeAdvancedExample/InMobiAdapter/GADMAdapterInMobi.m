...
BOOL isAppInstallRequest = NO;
BOOL isNativeContentRequest = NO;
BOOL shouldDownloadImages = YES;
BOOL serveAnyAd = NO;
InMobiMediatedNativeAppInstallAd *installAd;
GADInMobiExtras *extraInfo;
static NSCache *imageCache;

__attribute__((constructor))
static void initialize_imageCache() {
...
- (void)prepareRequestParameters {
    ...
    if ([self.connector userGender] == kGADGenderMale || [self.rewardedConnector userGender] == kGADGenderMale) {
        [IMSdk setGender:kIMSDKGenderMale];
    } else if ([self.connector userGender] == kGADGenderFemale || [self.rewardedConnector userGender] == kGADGenderFemale) {
    ...
    if(self.rewardedConnector){
        ...
        extraInfo= [self.connector networkExtras];
    }
    
    if (extraInfo != nil) {
        if(extraInfo.postalCode!=nil)
            [IMSdk setPostalCode:extraInfo.postalCode];
        if(extraInfo.areaCode!=nil)
            [IMSdk setAreaCode:extraInfo.areaCode];
        if(extraInfo.interests!=nil)
            [IMSdk setInterests:extraInfo.interests];
        if(extraInfo.age!=nil)
            [IMSdk setAge:extraInfo.age];
        if(extraInfo.nationality!=nil)
            [IMSdk setNationality:extraInfo.nationality];
        if(extraInfo.income!=nil)
            [IMSdk setIncome:extraInfo.income];
        if(extraInfo.yearOfBirth!=nil)
            [IMSdk setYearOfBirth:extraInfo.yearOfBirth];
        ...
        if(extraInfo.language!=nil)
            [IMSdk setLanguage:extraInfo.language];
    }
    
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    if (extraInfo && extraInfo.additionalParameters) {
        dict = [NSMutableDictionary dictionaryWithDictionary:extraInfo.additionalParameters];
    }
...
- (void)getNativeAdWithAdTypes:(NSArray *)adTypes options:(NSArray *)options{
    ...
    if (placementId==-1) {
        ...
        NSString *errorDesc = [NSString stringWithFormat:@"[InMobi] Exception - Placement ID not specified."];
        NSDictionary *errorInfo = [NSDictionary dictionaryWithObjectsAndKeys:errorDesc,
    ...
    if([self.connector testMode]) {
        NSLog(@"[InMobi] Please enter your device ID in the InMobi console to recieve test ads from Inmobi");
    }
    
    for (NSString *adType in adTypes) {
        if ([adType isEqual:kGADAdLoaderAdTypeNativeContent]) {
            isNativeContentRequest = YES;
        } else if ([adType isEqual:kGADAdLoaderAdTypeNativeAppInstall]) {
            isAppInstallRequest = YES;
        }
    }
...
- (void)getInterstitial {
    long long placementId = self.placementId;
    //Hardcoded placement for simplifying testing
    placementId=1505388360146;
    if (placementId==-1) {
        NSString *errorDesc = [NSString stringWithFormat:@"[InMobi] Exception - Placement ID not specified."];
        NSDictionary *errorInfo = [NSDictionary dictionaryWithObjectsAndKeys:errorDesc,
                                   NSLocalizedDescriptionKey, nil];
...
- (void)getBannerWithSize:(GADAdSize)adSize {
    ...
    if (GADAdSizeEqualToSize(adSize, kGADAdSizeBanner)) {
        self.adView = [[IMBanner alloc] initWithFrame:CGRectMake(0, 0, 320, 50) placementId:placementId];//self.placementId is hardcoded
    } else if (GADAdSizeEqualToSize(adSize, kGADAdSizeMediumRectangle)) {
        self.adView = [[IMBanner alloc] initWithFrame:CGRectMake(0, 0, 300, 250) placementId:placementId];//self.placementId is hardcoded
    } else if (GADAdSizeEqualToSize(adSize, kGADAdSizeFullBanner)) {
        self.adView = [[IMBanner alloc] initWithFrame:CGRectMake(0, 0, 468, 60) placementId:placementId];//self.placementId is hardcoded
    } else if (GADAdSizeEqualToSize(adSize, kGADAdSizeLeaderboard)) {
        self.adView = [[IMBanner alloc] initWithFrame:CGRectMake(0, 0, 728, 90) placementId:placementId];//self.placementId is hardcoded
    } else if (GADAdSizeEqualToSize(adSize, kGADAdSizeSkyscraper)) {
        self.adView = [[IMBanner alloc] initWithFrame:CGRectMake(0, 0, 120, 600) placementId:placementId];//self.placementId is hardcoded
    } else if ((GADAdSizeEqualToSize(adSize, kGADAdSizeSmartBannerPortrait)) || (GADAdSizeEqualToSize(adSize, kGADAdSizeSmartBannerLandscape))) {
        [self getOptimalSlotSize];
        self.adView = [[IMBanner alloc] initWithFrame:CGRectMake(0, 0, self.width, self.height) placementId:placementId];//self.placementId is hardcoded
    } else {
        NSString *errorDesc = [NSString stringWithFormat:@"[InMobi] Exception - Invalid ad type %@",
...
- (void)setUp{
    if(self)
        ...
        NSString *errorDesc = [NSString stringWithFormat:@"[InMobi] Exception - Error at the time of setting up adapter"];
        NSDictionary *errorInfo = [NSDictionary dictionaryWithObjectsAndKeys:errorDesc,
                                   NSLocalizedDescriptionKey, nil];
        GADRequestError *error = [GADRequestError errorWithDomain:kGADErrorDomain
...
- (void) getOptimalSlotSize {
    ...
    [dataArray insertObject:[NSMutableArray arrayWithObjects:[NSNumber numberWithInt:728],[NSNumber numberWithInt:90],nil] atIndex:0];
    [dataArray insertObject:[NSMutableArray arrayWithObjects:[NSNumber numberWithInt:468],[NSNumber numberWithInt:60],nil] atIndex:1];
    [dataArray insertObject:[NSMutableArray arrayWithObjects:[NSNumber numberWithInt:320],[NSNumber numberWithInt:50],nil] atIndex:2];
    
    for (int i=0; i < [dataArray count]; i++) {
        if (([[[dataArray objectAtIndex:i] objectAtIndex:0] intValue] <= screenWidth) && ([[[dataArray objectAtIndex:i] objectAtIndex:1] intValue]) <= screenHeight) {
...
- (void)presentInterstitialFromRootViewController:(UIViewController *)rootViewController {
    if ([self.interstitial isReady]) {
        [self.interstitial showFromViewController:rootViewController withAnimation:kIMInterstitialAnimationTypeCoverVertical];
...
- (long long)placementId {
    ...
    else if(self.rewardedConnector!=nil && [[self.rewardedConnector credentials] objectForKey:@"placementid"]){
...
-(void)interstitial:(IMInterstitial*)interstitial rewardActionCompletedWithRewards:(NSDictionary*)rewards {
    ...
    if(self.rewardedConnector!=nil){
        GADAdReward *reward = [[GADAdReward alloc] initWithRewardType:key rewardAmount:[rewards objectForKey:key]];
...
-(void)nativeDidFinishLoading:(IMNative*)native{
    ...
    installAd= [[InMobiMediatedNativeAppInstallAd alloc]initWithInMobiNativeAppInstallAd:native withAdapter:self shouldDownloadImage:shouldDownloadImages withCache:imageCache];
...
-(void)nativeWillPresentScreen:(IMNative*)native{
    ...
    [GADMediatedNativeAdNotificationSource mediatedNativeAdWillPresentScreen:installAd];
}
...
-(void)nativeWillDismissScreen:(IMNative*)native{
    ...
    [GADMediatedNativeAdNotificationSource mediatedNativeAdWillDismissScreen:installAd];
}
...
-(void)nativeDidDismissScreen:(IMNative*)native{
    ...
    [GADMediatedNativeAdNotificationSource mediatedNativeAdDidDismissScreen:installAd];
}
...
-(void)nativeAdImpressed:(IMNative *)native{
    ...
    [GADMediatedNativeAdNotificationSource mediatedNativeAdDidRecordImpression:installAd];
}
